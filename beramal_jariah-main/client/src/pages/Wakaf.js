...
function Wakaf() {
  ...
  const [wakafPrograms, setWakafPrograms] = useState({
    data: [],
    totalDana: 0,
    totalProgram: 0,
    load: true,
  ...
  const getData = async () => {
    try {
      const dataPrograms = await axios
        .get("http://localhost:5000/programs?type=wakaf")
      ...
      {wakafPrograms.load ? (
        ...
        <>
          <MainProgramInformationHeader
            ...
            totalProgram={wakafPrograms.totalProgram}
          />
